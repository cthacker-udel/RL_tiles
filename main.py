...
class ParsedInput:
    ...
    def print_policies(self: ParsedInput, board: list[list[BoardTile]]) -> str:
        ...
        sorted_board_tiles = dict(sorted(board_tiles.items()))
        joined_strings = []
        for each_key in sorted_board_tiles:
    ...
    def q_value(self: State) -> None:
        ...
        while self.run:  # while convergence is not triggered
            ...
            while not current_tile.is_terminal:  # while the current tile is not a terminal tile
                if iteration_count > self.max_iter and self.run:  # if the iteration count exceeds the thresholds
                ...
                try:
                    ...
                    # if future tile is terminal node, then set right side max_a Q(s', a') to 0
                    if future_tile.is_terminal:
                        right_side_max_q = 0  # IF TERMINAL Q(s', a') = 0
            ...
            if self.debug or iteration_count % 10_000 == 0:  # if debug is on or the iteration count is a multiple of 10_000
...
def main(inp: bool = False):
    if inp:
        ...
        test_case = TestCase()
        for ind, each_test_case in enumerate(test_cases):
